...
async def llm_call_text(prompt: str, model_id: Optional[str] = None) -> str:
    """
    用于生成纯文本/Markdown 摘要的 LLM 调用（不强制要求 JSON 格式）。
    ...
    try:
        ...
        data = {
            ...
            # 关键：这里不包含 'response_format' 字段，以接收纯文本
            'temperature': 0.7,
        ...
        print(f'\n❌ LLM (Text) API 调用失败: {e.response.status_code} 状态码错误')
        print(f'   错误详情: {error_details}', file=sys.stderr)
        raise
    except httpx.RequestError as e:
        print(f'\n❌ LLM (Text) API 调用失败: 请求错误 ({e})', file=sys.stderr)
        raise
    except Exception as error:
        print(f'\n❌ LLM (Text) API 调用失败: {error}', file=sys.stderr)
        raise